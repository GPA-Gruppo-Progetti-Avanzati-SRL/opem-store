...
func FindByPk(collection *mongo.Collection, mustFind bool, findOptions *options.FindOneOptionsBuilder) (*model.Session, bool, error) {
	const semLogContext = "session::find-by-pk"
...
func Find(collection *mongo.Collection, f *Filter, withCount bool, findOptions *options.FindOptionsBuilder) (model.SessionQueryResult, error) {
	const semLogContext = "session::find"
	fd := f.Build()
