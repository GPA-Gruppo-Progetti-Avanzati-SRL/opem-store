...
func FindBySId(collection *mongo.Collection, sid string, mustFind bool, findOptions *options.FindOneOptionsBuilder) (*model.Session, error) {
...
func Insert(ctx context.Context, aCollection *mongo.Collection, s *model.Session) (string, error) {
	const SemLogContext = "opem-core/session/insert"
	log.Trace().Str("user-id", s.Userid).Msg(SemLogContext)

	now := time.Now()
	s.SysInfo.Status = "active"
	s.SysInfo.CreatedAt = now
	s.SysInfo.ModifiedAt = now
	...
	s.OId = r.InsertedID.(bson.ObjectID).Hex()
	sid := s.SessionId()
	log.Trace().Str("sid", sid).Msg(SemLogContext)
...
func UpdateBySid(ctx context.Context, aCollection *mongo.Collection, sid string, mustMatch bool, opts ...UpdateOption) (int, error) {
	const SemLogContext = "opem-core/session/update-by-sid"
	log.Trace().Str("sid", sid).Msg(SemLogContext)
