...
func FindByPk(collection *mongo.Collection, bid string, mustFind bool, findOptions *options.FindOneOptionsBuilder) (*ObjectStoreEvent, bool, error) {
	// @tpm-schematics:end-region("find-by-pk-signature-section")
	const semLogContext = "object-store-event::find-by-pk"
...
func Find(collection *mongo.Collection, f *Filter, withCount bool, findOptions *options.FindOptionsBuilder) (QueryResult, error) {
	const semLogContext = "object-store-event::find"
	fd := f.Build()
