...
type Card struct {
	...
	CardNumber            string              `json:"card_number,omitempty" bson:"card_number,omitempty" yaml:"card_number,omitempty"`
	...
	EnvelopeNumber        string              `json:"envelope_number,omitempty" bson:"envelope_number,omitempty" yaml:"envelope_number,omitempty"`
...
func (s Card) IsZero() bool {
	return s.OId == bson.NilObjectID && s.Domain == "" && s.Site == "" && s.Bid == "" && s.Et == "" && s.CardNumber == "" && s.CardType == "" && s.Status == "" && s.IdCardExt == "" && s.EnvelopeNumber == "" && s.Funct == "" && s.FocalPoint.IsZero() && s.Product.IsZero() && s.Magazzino.IsZero() && s.Holder.IsZero() && len(s.Apps) == 0 && len(s.Addresses) == 0 && s.ExpiresAt == 0 && s.IssueDate == 0 && s.IssueConfirmationDate == 0 && s.ActDate == 0 && s.SysInfo.IsZero()
}
