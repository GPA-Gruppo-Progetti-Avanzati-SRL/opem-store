	...
	pipeline = append(pipeline, bson.D{
		{"$lookup", bson.D{
			...
			{"let", bson.D{
				...
				{"site", "$site"},
				{"et", card.EntityType},
				{"bid_card_from", bson.D{{"$ifNull", bson.A{"$card_bids_range.from", ""}}}},
				{"bid_card_to", bson.D{{"$ifNull", bson.A{"$card_bids_range.to", ""}}}},
			}},
			{"pipeline", bson.A{
				bson.D{
					{"$match", bson.D{
